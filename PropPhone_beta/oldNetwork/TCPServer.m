...
- (BOOL)start:(NSError **)error {
    ...
    prop_socket = CFSocketCreate(kCFAllocatorDefault, PF_INET6, SOCK_STREAM, IPPROTO_TCP, kCFSocketAcceptCallBack, (CFSocketCallBack)&TCPServerAcceptCallBack, &socketCtxt);
	
	if (prop_socket != NULL)	// the socket was created successfully
	{
		protocolFamily = PF_INET6;
	} else // there was an error creating the IPv6 socket - could be running under iOS 3.x
	{
		prop_socket = CFSocketCreate(kCFAllocatorDefault, PF_INET, SOCK_STREAM, IPPROTO_TCP, kCFSocketAcceptCallBack, (CFSocketCallBack)&TCPServerAcceptCallBack, &socketCtxt);
		if (prop_socket != NULL)
		{
    ...
    if (NULL == prop_socket) {
        ...
        if (prop_socket) CFRelease(prop_socket);
        prop_socket = NULL;
        return NO;
    }
    ...
    int yes = 1;
    setsockopt(CFSocketGetNative(prop_socket), SOL_SOCKET, SO_REUSEADDR, (void *)&yes, sizeof(yes));
	...
	if (protocolFamily == PF_INET6)
	{
		...
		if (kCFSocketSuccess != CFSocketSetAddress(prop_socket, (CFDataRef)address6)) {
		...
		NSData *addr = [(NSData *)CFSocketCopyAddress(prop_socket) autorelease];
		...
		if (kCFSocketSuccess != CFSocketSetAddress(prop_socket, (CFDataRef)address4)) {
    ...
    CFRunLoopSourceRef source = CFSocketCreateRunLoopSource(kCFAllocatorDefault, prop_socket, 0);
...
- (BOOL)stop {
	...
	if (prop_socket) {
		CFSocketInvalidate(prop_socket);
		CFRelease(prop_socket);
		prop_socket = NULL;
	}
...
- (BOOL) enableBonjourWithDomain:(NSString*)domain applicationProtocol:(NSString*)protocol name:(NSString*)name
{
	...
	if(!protocol || ![protocol length] || prop_socket == NULL)
		return NO;
